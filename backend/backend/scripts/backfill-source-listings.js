...
const apply = typeof arguments !== 'undefined' && arguments.includes('--apply');
...
db.jobs.find(query).forEach(job => {
  ...
  if ((job.sourceIdentities || []).length > 1 && !job.sourceIdentities.includes(identity)) { ambiguous++; printjson({ambiguous: job._id, identity}); return; }
  candidates++;
