...
class OpenApiSmokeTest {
    ...
    void openApiPublishesVersionedDtoContractsAndSecuritySchemes() throws Exception {
        mvc.perform(get("/v3/api-docs")).andExpect(status().isOk())
                ...
                .andExpect(jsonPath("$.paths['/api/v1/candidate-profile']").exists())
                .andExpect(jsonPath("$.paths['/api/v1/candidate-profile/resume']").exists())
                .andExpect(jsonPath("$.paths['/api/v1/candidate-profile/resume/status']").exists())
                .andExpect(content().string(containsString("Candidate Intelligence")))
